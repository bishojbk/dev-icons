...
export class AlpinelinuxIconElement extends HTMLElement {
  ...
  private render() {
    ...
    const color = this.getAttribute('color') || '#0D597F';
...
export function registerAlpinelinuxIcon(name = 'devicon-alpinelinux') {
  if (!customElements.get(name)) {
    customElements.define(name, AlpinelinuxIconElement);
  }
}
