  ...
  return (
    ...
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2.5 font-bold text-lg select-none">
          ...
          <span style={{ color: 'var(--text-primary)' }}>DevIcon Kit</span>
        </Link>
      ...
      {/* Mobile Nav */}
      {mobileOpen && (
        <nav className="border-t px-4 py-3 md:hidden" style={{ borderColor: 'var(--border)' }}>
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
